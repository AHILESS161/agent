...
Поиск по реестру товарных знаков работает на **ограниченном демо-датасете**
(55 записей). Это явно отмечается в интерфейсе и отчётах. Реальная
интеграция с ФИПС не реализована, и подмена её демо-данными запрещена:
`ProviderFactory.create("fips")` поднимает ошибку, а не возвращает mock.
