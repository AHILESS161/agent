...
### POST `/applications/{id}/classes/{class_id}/narrow`

Формирует, но не сохраняет модельное предложение по сужению полного перечня
класса. В модель передаются описание деятельности, товары/услуги и
пронумерованные официальные позиции. Ответ содержит `source_count`,
`selected_count`, `selected_items`, `proposed_description`, `rationale` и
`assumptions`. Все `selected_items` дословно принадлежат справочнику МКТУ.

### POST `/applications/{id}/classes/{class_id}/narrow/apply`

Сохраняет подтверждённый пользователем результат предпросмотра. Тело:
`{ "selected_items": ["точная позиция справочника"] }`. Сервер повторно
проверяет каждую позицию по официальному перечню и не меняет статус подтверждения
класса. Неизвестная или пустая позиция возвращает `422`.
...
### GET `/api/v1/applications/{application_id}/draft-form`

Возвращает структурированный предпросмотр официального бланка. Каждое поле
содержит `source`, `field_path`, `fill`, `editable`, `multiline`, статус,
источник значения и признак обязательности.

### POST `/api/v1/applications/{application_id}/fields`

Сохраняет ручную правку из предпросмотра. Тело содержит `field_path`, `label`,
`value` и `is_sensitive`. Для поддерживаемых источников endpoint одновременно
обновляет основное поле заявки или карточки заявителя; служебная запись хранит
происхождение и историю решения. После ручной правки прежнее подтверждение
данных становится недействительным.

Перечень `application.goods_services` этим методом не правится: он изменяется
через endpoints классов МКТУ, чтобы не потерять номер класса и объём охраны.
