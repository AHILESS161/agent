...
- `client` видит только свои заявки (`applications.client_id` привязан к его `Client`).
- `manager` видит только заявки своих клиентов (`clients.manager_id == current_user.id`).
- `lawyer` видит заявки, где `responsible_lawyer_id == current_user.id` или все (конфигурируемо).
- `admin` видит всё.
