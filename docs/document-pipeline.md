...
> **Статус на 16.08.2026:** частично реализовано. Работают предпросмотр полей,
> рабочая DOCX-выгрузка, версии утверждаемого черновика и ZIP-пакет обычной
> самостоятельной заявки. Полный пакет для всех специальных сценариев, подпись
> и электронная подача остаются целевым дизайном. См. [`current-state.md`](current-state.md).
...
│   ├── 04_{доверенность}                # при представительстве
│   └── 05_{документ_о_приоритете}       # при заявленном приоритете
├── 02_ДЛЯ_ВАС/
