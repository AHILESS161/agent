...
export function OfficeActionResponse({ appId }: { appId: number }) {
  ...
  return <div>
    ...
    <h2 className="mt-2 text-3xl font-semibold text-[#11113f]">Ответить Роспатенту</h2>
    <p className="mt-3 max-w-3xl leading-relaxed text-[#6d6d7d]">Загрузите уведомление и сообщите только те обстоятельства, которые можете подтвердить. Система объяснит замечания и подготовит редактируемый черновик ответа.</p>
    ...
    <FactGroup title="2. Почему товары или услуги отличаются" description="Отметьте только подходящие пункты. Сам номер класса МКТУ не доказывает, что товары однородны." items={homogeneity} group="homogeneity" documents={documents} onChange={updateFact} onUpload={uploadEvidence} />
    <FactGroup title="3. Как знак стал узнаваемым" description="Этот блок нужен, если обозначение использовалось до подачи заявки. Добавьте конкретные даты, показатели и материалы." items={distinctiveness} group="distinctiveness" documents={documents} onChange={updateFact} onUpload={uploadEvidence} />
