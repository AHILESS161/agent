...
> **Статус на 15.08.2026:** частично реализовано. Работают предпросмотр полей,
> рабочая DOCX-выгрузка и версии утверждаемого черновика. Описанный ниже полный
> пакет для всех сценариев, подпись и электронная подача остаются целевым
> дизайном. См. [`current-state.md`](current-state.md).
...
Защищённая финальная версия требует утверждения специалистом. Универсальный
полный пакет для фактической подачи пока не формируется.
...
| `power_of_attorney` | Доверенность патентного поверенного | Требуется при представительстве | 1.1 |
...
class TemplateSelector:
    ...
    def select(self, data: ApplicationDocumentData) -> list[str]:
        ...
        # Доверенность нужна, если есть представитель (не сам заявитель)
