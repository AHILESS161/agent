...
> **Статус документа:** целевой дизайн безопасности, а не акт соответствия
> 152-ФЗ и не описание всех работающих контролей. Фактическая модель доступа и
> эксплуатационные ограничения приведены в
> [`current-state.md`](current-state.md#безопасность-и-эксплуатация).

> **Версия:** 1.1
> **Дата:** 2026-08-28
>
> Фактический аудит персональных данных, реализованное удаление и оставшиеся
> обязательные решения до пилота описаны в
...
- Хранилище аудита (`AuditLog`) не шифрует JSON на уровне приложения. Текущая
  реализация не должна записывать туда значения чувствительных полей; защита
  данных в покое обеспечивается production-инфраструктурой.
- Основной LLM-анализ получает минимальный предметный контекст без реквизитов
  заявителя. Исключения (vision и необязательный LLM-разбор бланка) требуют
  отдельного уведомления и основания обработки.
...
- Записи приложения не редактируются через API; неизменяемость на уровне БД
  отдельно не обеспечена.
- Универсального пятилетнего срока в 152-ФЗ нет. Срок задаётся целью обработки,
  законом или договором и должен быть утверждён оператором по категориям.
- Доступ: `admin` видит общий журнал; `lawyer` — только журнал созданных или
  назначенных ему дел.
